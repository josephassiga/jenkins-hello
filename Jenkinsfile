            ...
            stage('Checkout.')
            {
                ...
                script {
                   /* switch(params.ENVIRONMENT) {
                    ...
                    APP_IMAGE_TAG = "${env.ENVIRONMENT == 'OP' ? 'PromoteToOP' : env.ENVIRONMENT == 'AP' ? 'PromoteToAP' : sh (script: "git log -n 1 --pretty=format:'%h'", returnStdout: true).trim() }"
                }
                    //script 
                    //{
                                ...
                                echo "The Image tag used is : ${sh 'printenv'}"
                                echo "**************************************************"
                   // } // script
                } // steps
            } // stage
